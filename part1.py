...
ht=sns.distplot(df["height"])
ht.set(xlabel="Height", title="Distribution by Height")

# %%
wt= sns.distplot(df["weight"])
wt.set(xlabel="Height", title="Distribution by weight")
...
# %% plot month
plt=sns.countplot(df["datetime"].dt.month)
plt.set(xlabel="Months", title="# of incidents by Months")
...
ax.set(xlabel="hour", title="# of incidents by hour")
# %% plot xcoord / ycoord
...
# %% plot race
rc= sns.countplot(data=df, y="race")
rc.set(ylabel="race", title="# of incidents by race")

# %% plot race wrt city
rcs=sns.countplot(data=df, y="race", hue="city")

rcs.set(ylabel="race", title="# of incidents by race and city")
...
pltc.set(ylabel="Physica force area", title="Top crime where Physical Fores used")

# %% plot percentage of each physical forces used
pfs = [col for col in df.columns if col.startswith("pf_")]
pf_counts = (df[pfs] == "YES").sum()
pltpf=sns.barplot(y=pf_counts.index, x=pf_counts.values / df.shape[0])
pltpf.set(ylabel="Physica force area", title="percentage of Physical Fores used")
